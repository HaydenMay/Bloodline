...
import { getNextPrestigeUnlock } from '../data/unlocks.js';
...
export function mountStableHub(
  ...
  // A bare number says nothing about why the player should want more of it.
  // Naming the next wall turns prestige from a mystery into a target.
  const nextUnlock = getNextPrestigeUnlock(stablePoints, career.stable.staff);
  ...
  root.innerHTML = `
    <div class="hub-container">
      <!-- Two numbers, each saying what it is for. There used to be three, and
           two of them measured the same thing. -->
      <div class="hub-status-bar">
        <div class="status-item">
          ...
          <span class="status-note">Pays for facilities, staff and supplies</span>
        </div>
        <div class="status-item">
          <span class="status-label">Prestige</span>
          <span class="status-value">${stablePoints.toLocaleString()}</span>
          <span class="status-note">${
            nextUnlock
              ? `Unlocks ${nextUnlock.what} at ${nextUnlock.at.toLocaleString()}`
              : 'Everything unlocked'
          }</span>
        </div>
      </div>
